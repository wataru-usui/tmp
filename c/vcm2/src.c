...
bool box_isect(BOX box, VEC o, VEC d) {
  ...
  return tmax >= tmin && fmax(tmin, tmax) > eps;
}
...
void sample_eye(CONF conf, PRNG_STATE *s, int ix, int iy,
FACE **f, VEC *n, VEC *o, VEC *d, VEC *fs, double *pa, double *ps, bool *c) {
  *f = NULL;
  *n = vec_init(0.0, 0.0, -1.0);
  *o = disc_sample(conf.r, s);
  *d = vec_init(conf.wf * (0.5 - (ix + prng_db(s)) / conf.wi), conf.hf * ((iy + prng_db(s)) / conf.hi - 0.5), conf.df);
...
void vcm(CONF conf, SCENE scene, IMAGE img, PRNG_STATE *states) {
  image_fill(img, 0.0);
  int N = conf.num_samples * conf.wi * conf.hi;
  ...
  for (int iter_num = 0; iter_num < conf.num_iters; ++iter_num) {
    ...
    double r = conf.ir * pow(iter_num + 1, -1.0 / 12.0);
    t = time_get();
    #pragma omp parallel
    {
    ...
    #pragma omp parallel
    {
      ...
      for (int i = omp_get_thread_num(); i < img.w * img.h; i += omp_get_max_threads()) {
        ...
        for (int j = 0; j < conf.num_samples; ++j) {
          PATH *pl = pls + conf.num_samples * i + j;
          ...
          sum0 = vec_add(sum0, vm(N, r, num_kdt_nodes, nodes, *pe));
          sum0 = vec_add(sum0, vc(scene, N, r, *pl, *pe));
        }
        sum0 = vec_scale(1.0 / N, sum0);
        img.data[3 * i + 0] += sum0.x;
        img.data[3 * i + 1] += sum0.y;
