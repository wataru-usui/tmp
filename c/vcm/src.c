...
typedef struct {
  int num_threads, num_samples, num_objs, num_verts;
  int wi, hi;
  double ef, wf, hf, df;
  double r, f;
  OBJECT *objs;
...
void orthogonal_basis(DOUBLE3 *x, DOUBLE3 *y, DOUBLE3 z) {
  if (fabs(z.x) > fabs(z.y)) {
    *x = double3_scale(1.0 / sqrt(sqr(z.x) + sqr(z.z)),
    double3_init3(-z.z, 0.0, z.x));
  } else {
    *x = double3_scale(1.0 / sqrt(sqr(z.y) + sqr(z.z)),
    double3_init3(0.0, z.z, -z.y));
  }
  *y = double3_cross(z, *x);
}
...
DOUBLE3 refr(DOUBLE3 n, DOUBLE3 d, double eta, double cosi, double cost) {
  return double3_sub(double3_scale(eta, d),
  double3_scale(eta * cosi - cost, n));
}
...
double fresnel_conductor_approx(double eta, double k, double cosi) {
  ...
  double perp = (a * sqr(cosi) - 2.0 * eta * cosi + 1.0) /
  (a * sqr(cosi) + 2.0 * eta * cosi + 1.0);
  double paral = (a - 2.0 * eta * cosi + sqr(cosi)) /
  (a + 2.0 * eta * cosi + sqr(cosi));
...
double fresnel_conductor(double eta, double k, double cosi) {
  ...
  double perp = (w + sqr(cosi) - 2.0 * a * cosi) /
  (w + sqr(cosi) + 2.0 * a * cosi);
  double paral = (sqr(cosi) * w + sqr(s) - 2.0 * a * cosi * s) /
  (sqr(cosi) * w + sqr(s) + 2.0 * a * cosi * s);
...
DOUBLE3 sphere_uniform_sample(SPHERE s, PRNG_STATE *state) {
  ...
  cosp = cos(phi);
  sinp = sin(phi);
  return double3_add(s.c, double3_scale(s.r,
  spherical_coord(cost, sint, cosp, sinp)));
}
...
DOUBLE3 bsdf_eval(BSDF bsdf, double ni, double nt,
DOUBLE3 n, DOUBLE3 wi, DOUBLE3 wo) {
  switch (bsdf) {
    case BSDF_NULL : {
    ...
    case BSDF_CONDUCTOR : {
      ...
      return double3_init(double3_equal(wo, r) ?
      fresnel_conductor(ni / m, k, cosi) / fabs(cosi) : 0.0);
    }
...
double bsdf_pdf(BSDF bsdf, double ni, double nt,
DOUBLE3 n, DOUBLE3 wi, DOUBLE3 wo) {
  switch (bsdf) {
    case BSDF_NULL : {
...
DOUBLE3 bsdf_sample(BSDF bsdf, double ni, double nt, DOUBLE3 n, DOUBLE3 wi,
PRNG_STATE *state) {
  switch (bsdf) {
    case BSDF_NULL : {
      return wi;
...
MATERIAL material_init(DOUBLE3 e, DOUBLE3 s, EDF edf, BSDF bsdf,
MEDIUM mp, MEDIUM mn) {
...
bool intersect(int num_objs, OBJECT *objs, DOUBLE3 o, DOUBLE3 d,
int *id, double *t) {
  *t = INFINITY;
...
bool kdtree_debug(int a, int b, NODE *nodes, int depth) {
  if (b - a <= 1) {
    return true;
  }
  bool res = false;
  ...
  NODE nr = nodes[ir];
  PATH *paths = nm.paths;
  VERTEX vm = paths[nm.i].vs[nm.j];
  VERTEX vl = paths[nl.i].vs[nl.j];
  VERTEX vr = paths[nr.i].vs[nr.j];
  int axis = depth % 3;
  switch (axis) {
    case 0 : {
      if (vl.x.x < vm.x.x && (vr.x.x > vm.x.x || ir == b)) {
        res = true;
      }
    } break;
    case 1 : {
      if (vl.x.y < vm.x.y && (vr.x.y > vm.x.y || ir == b)) {
        res = true;
      }
    } break;
    case 2 : {
      if (vl.x.z < vm.x.z && (vr.x.z > vm.x.z || ir == b)) {
        res = true;
      }
    } break;
    default : {
  ...
  res = res && kdtree_debug(a, im, nodes, depth + 1);
  res = res && kdtree_debug(im + 1, b, nodes, depth + 1);
  return res;
}
...
OBJECT *objs, int N, double r, PATH pe, VERTEX ve, int je, DOUBLE3 *sum) {
  ...
  //if (dist < r && fabs(double3_dot(ve.n, d)) <= 1.0 * r) {
  if (dist < r) {
    ...
    if (jl >= 2) {
      double p = 1.0;
      {
        OBJECT objlm1 = objs[vlm1.id];
        bool clm1 = !bsdf_delta(objlm1.m.bsdf);
        p *= pbl / vl.pf;
        if (clm1) {
          w += p;
        }
      ...
      for (int kl = jl - 2; kl >= 1; --kl) {
        VERTEX vl = pl.vs[kl];
        OBJECT objl = objs[vl.id];
        bool cl = !bsdf_delta(objl.m.bsdf);
        VERTEX vlp1 = pl.vs[kl + 1];
        p *= vl.pb / vlp1.pf;
        if (cl) {
          w += p;
        }
    ...
    if (je >= 2) {
      double p = 1.0;
      {
        OBJECT objem1 = objs[vem1.id];
        bool cem1 = !bsdf_delta(objem1.m.bsdf);
        p *= pbe / ve.pf;
        if (cem1) {
          w += p;
        }
      ...
      for (int ke = je - 2; ke >= 1; --ke) {
        VERTEX ve = pe.vs[ke];
        OBJECT obje = objs[ve.id];
        bool ce = !bsdf_delta(obje.m.bsdf);
        VERTEX vep1 = pe.vs[ke + 1];
        p *= ve.pb / vep1.pf;
        if (ce) {
          w += p;
        }
    ...
    if (jl == 1) {
      ...
      OBJECT objlm1 = objs[vlm1.id];
      bool clm1 = !edf_delta(objlm1.m.edf);
      p *= 1.0 / vl.pf;
      if (clm1) {
        w += p;
      }
      ...
      bool clm1 = !bsdf_delta(objlm1.m.bsdf);
      VERTEX vlm2 = pl.vs[0];
      OBJECT objlm2 = objs[vlm2.id];
      bool clm2 = !edf_delta(objlm2.m.edf);
      p *= 1.0 / vl.pf;
      if (clm1) {
        w += p;
      }
      ...
      OBJECT objlm2 = objs[vlm2.id];
      bool clm2 = !bsdf_delta(objlm2.m.bsdf);
      p *= 1.0 / vl.pf;
      if (clm1) {
        w += p;
      }
      ...
      VERTEX vl1 = pl.vs[1];
      OBJECT objl1 = objs[vl1.id];
      bool cl1 = !bsdf_delta(objl1.m.bsdf);
      VERTEX vl0 = pl.vs[0];
      OBJECT objl0 = objs[vl0.id];
      bool cl0 = !edf_delta(objl0.m.edf);
      p *= vl1.pb / vl1.pf;
      if (cl1 && cl0) {
        w += p;
      }
    ...
    if (je == 1) {
      ...
      OBJECT objem2 = objs[vem2.id];
      bool cem2 = !bsdf_delta(objem2.m.bsdf);
      p *= 1.0 / ve.pf;
      if (cem1) {
        w += p;
      }
  ...
  for (int je = 1; je < pe.n; ++je) {
    ...
    if (ce) {
      ...
      
      {
        double p = 1.0;
        if (je >= 2) {
          OBJECT objem1 = scene.objs[vem1.id];
          bool cem1 = !bsdf_delta(objem1.m.bsdf);
          p *= pbe0 / ve.pf;
          if (cem1) {
            w += p;
        ...
        if (je >= 3) {
          ...
          bool cem1 = !bsdf_delta(objem1.m.bsdf);
          VERTEX vem2 = pe.vs[je - 2];
          OBJECT objem2 = scene.objs[vem2.id];
          bool cem2 = !bsdf_delta(objem2.m.bsdf);
          p *= pbe1 / vem1.pf;
          if (cem1 && cem2) {
            w += p;
        ...
        if (ce) {
          ...
          if (visible(scene.num_objs, scene.objs, ve.x, de, t)) {
            ...
            
            {
              double p = 1.0;
              if (jl == 1) {
                OBJECT objlm1 = scene.objs[vlm1.id];
                bool clm1 = !edf_delta(objlm1.m.edf);
                p *= pbl0 / vl.pf;
                if (clm1) {
                  w += p;
              ...
              if (jl == 1) {
                ...
                bool clm1 = !bsdf_delta(objlm1.m.bsdf);
                VERTEX vlm2 = pl.vs[0];
                OBJECT objlm2 = scene.objs[vlm2.id];
                bool clm2 = !edf_delta(objlm2.m.edf);
                p *= pbl1 / vlm1.pf;
                if (clm1 && clm2) {
                  w += p;
              ...
              if (jl == 2) {
                VERTEX vl0 = pl.vs[0];
                OBJECT objl0 = scene.objs[vl0.id];
                bool cl0 = !edf_delta(objl0.m.edf);
                p *= vl0.pb / vl0.pf;
                if (cl0) {
                  w += p;
                ...
                bool cl0 = !edf_delta(objl0.m.edf);
                VERTEX vl1 = pl.vs[1];
                OBJECT objl1 = scene.objs[vl1.id];
                bool cl1 = !bsdf_delta(objl1.m.bsdf);
                p *= vl1.pb / vl1.pf;
                if (cl1 && cl0) {
                  w += p;
...
void trace(SCENE scene, PRNG_STATE *s, PATH *path, int id, double pa, double ps,
DOUBLE3 n, DOUBLE3 o, DOUBLE3 d, DOUBLE3 fs, bool light) {
  ...
  for (int i = 1; i < scene.num_verts; ++i) {
    ...
    a = double3_scale(fabs(coso) / ps, double3_mul(double3_mul(
    a, obj.m.s), fs));
...
DOUBLE3 *n, DOUBLE3 *o, DOUBLE3 *d, DOUBLE3 *We, double *pa, double *ps) {
...
DOUBLE3 *n, DOUBLE3 *o, DOUBLE3 *d, DOUBLE3 *Le, double *pa, double *ps,
int *id) {
...
void pt(SCENE scene, IMAGE img, PRNG_STATE *s) {
  ...
  i += omp_get_max_threads()) {
    ...
    for (int i = 0; i < scene.num_samples; ++i) {
      DOUBLE3 n, o, d, We;
      double pa, ps;
      sample_eye(scene, s, ix, iy, &n, &o, &d, &We, &pa, &ps);
      trace(scene, s, &path, -1, pa, ps, n, o, d, We, false);
      DOUBLE3 sum1 = double3_init(0.0);
      for (int i = 1; i < path.n; ++i) {
        VERTEX v = path.vs[i];
        OBJECT obj = scene.objs[v.id];
        DOUBLE3 Le = double3_mul(obj.m.e, edf_eval(
        obj.m.edf, v.n, double3_scale(-1.0, v.wi)));
    ...
    if (omp_get_thread_num() == 0) {
      //printf("progress: %f\r", i / CAST(double, img.w * img.h));
    }
  }
}
...
void vcm(SCENE scene, IMAGE img, PRNG_STATE *states) {
  ...
  for (int i = 0; i < N; ++i) {
    pls[i].vs = ALLOC(VERTEX, scene.num_verts);
  }
  PATH pes[omp_get_max_threads()];
  for (int i = 0; i < omp_get_max_threads(); ++i) {
    pes[i].vs = ALLOC(VERTEX, scene.num_verts);
  }
  NODE *nodes = ALLOC(NODE, (scene.num_verts - 1) * N);
  for (int iter_num = 0; iter_num < scene.num_iters; ++iter_num) {
    printf("number of iterations: %d\n", iter_num);
    double r = scene.init_r * pow(iter_num + 1, (0.75 - 1.0) / 2.0);
    #pragma omp parallel
    {
      PRNG_STATE *s = states + omp_get_thread_num();
      for (int i = omp_get_thread_num(); i < N;
      i += omp_get_max_threads()) {
        PATH *pl = pls + i;
        DOUBLE3 n, o, d, Le;
        double pa, ps;
        int id;
        sample_light(scene, s, &n, &o, &d, &Le, &pa, &ps, &id);
        trace(scene, s, pl, id, pa, ps, n, o, d, Le, true);
      }
    }
    int num_nodes = 0;
    for (int i = 0; i < N; ++i) {
    ...
    printf("kd tree construction begin\n");
    kdtree_construct(0, num_nodes, nodes, 0);
    printf("kd tree construction end\n");
    //printf("kd tree debug begin\n");
    //printf("%kd tree debug %d\n", kdtree_debug(0, num_nodes, nodes, 0));
    //printf("kd tree debug end\n");
    #pragma omp parallel
    {
      PRNG_STATE *s = states + omp_get_thread_num();
      for (int i = omp_get_thread_num(); i < img.w * img.h;
...
SCENE scene_init1(void) {
  ...
  double r = 15.0, theta = 60.0 * PI / 180.0, phi = 180.0 * PI / 180.0;
  ...
  scene.objs[1] =
  object_init(material_init(
  double3_scale(1.0e+3 / (4.0 * PI * sqr(1.0e-6)), double3_init3(255 / 255.0, 255 / 255.0, 255 / 255.0)),
  double3_init3(1.0, 1.0, 1.0),
  EDF_UNI, BSDF_NULL,
  m,
  m),
  ...
  for (int i = 0; i < nh; ++i) {
    for (int j = 0; j < nt; ++j) {
      ...
      MEDIUM mn;
      mn.n = 1.3 + 0.4 * prng_next_double(&state);
      mn.a.x = -1.0 * log(1.0e-3 + prng_next_double(&state));
      mn.a.y = -1.0 * log(1.0e-3 + prng_next_double(&state));
      mn.a.z = -1.0 * log(1.0e-3 + prng_next_double(&state));
      OBJECT obj;
...
SCENE scene_init3(void) {
  ...
  scene.objs[6] =
  object_init(material_init(
  double3_scale(10.0 / (4.0 * PI * sqr(0.001)), double3_init3(0.3, 0.3, 0.9)),
  double3_init3(1.0, 1.0, 1.0),
  EDF_UNI, BSDF_NULL,
  m,
  medium_init(1.0, double3_init3(0.0, 0.0, 0.0))),
  sphere_init(0.001, double3_init3((w / 2.0 - 0.001) / 2.0 - 0.5, h - 0.001, -2.0)));
  scene.objs[7] =
  object_init(material_init(
  double3_scale(10.0 / (4.0 * PI * sqr(0.001)), double3_init3(0.9, 0.3, 0.3)),
  double3_init3(1.0, 1.0, 1.0),
  EDF_UNI, BSDF_NULL,
  m,
  medium_init(1.0, double3_init3(0.0, 0.0, 0.0))),
  sphere_init(0.001, double3_init3((0.001 - w / 2.0) / 2.0 + 0.5, h - 0.001, -2.0)));
  scene.objs[8] =
  object_init(material_init(
  double3_init3(0.0, 0.0, 0.0),
  double3_init3(1.0, 1.0, 1.0),
  EDF_UNI, BSDF_DIELECTRIC,
  m,
  medium_init(1.5, double3_init3(3.0, 3.0, 9.0))),
  sphere_init(0.3, double3_init3((w / 2.0 - 0.3) / 2.0 - 0.5, 0.3 - h, -2.25)));
  scene.objs[9] =
  object_init(material_init(
  double3_init3(0.0, 0.0, 0.0),
  ...
  EDF_UNI, BSDF_DIELECTRIC,
  m,
  medium_init(1.7, double3_init3(3.0, 1.0, 1.0))),
  sphere_init(0.3, double3_init3((0.3 - w / 2.0) / 2.0 + 0.5, 0.3 - h, -2.0)));
  return scene;
}
